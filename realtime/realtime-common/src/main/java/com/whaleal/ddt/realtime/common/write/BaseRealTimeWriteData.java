 ...
 */
package com.whaleal.ddt.realtime.common.write;
...
public  abstract class BaseRealTimeWriteData<T> extends CommonTask {
        ...
        while (true) {
            try {
                ...
                for (Map.Entry<Integer, BlockingQueue<BatchDataEntity>> next : metadata.getQueueOfBucketMap().entrySet()) {
                    ...
                    AtomicBoolean atomicBoolean = metadata.getStateOfBucketMap().get(bucketNum);
    ...
    public void bulkExecute(BatchDataEntity batchDataEntity) {
        ...
        try {
            ...
            if (metadata.getUniqueIndexCollection().containsKey(dbName + "." + tableName)) {
    ...
    private void bulkWriteInfo(BulkWriteResult bulkWriteResult) {
        ...
        metadata.updateBulkWriteInfo("insert", insertedCount);
        metadata.updateBulkWriteInfo("update", modifiedCount);
        metadata.updateBulkWriteInfo("delete", deletedCount);
    }
}
