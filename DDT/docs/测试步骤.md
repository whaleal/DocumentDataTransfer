...
参考[QuickStart](快速启动.md)

该测试环境使用如下参数

```

# mongodbT.properties 配置文件
#任务名。不写则默认生成
taskName=mongoTask
#程序名名。不写则默认生成
proName=mongoPro
...
源端CURD数据量过大时会造成MongoT无法及时同步源表Oplog。及时观察'读取oplog延迟xxxs'数据，避免读取oplog时，错过滑动窗口时间。
