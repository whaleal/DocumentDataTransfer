 ...
 */
package com.whaleal.ddt.sync.changestream.cache;
...
public final class MetadataEvent {
     ...
     */
    private static Map<String, MetadataEvent> eventMetadataMap = new ConcurrentHashMap<>();
     ...
     */
    public static MetadataEvent getEventMetadata(String workName) {
        return eventMetadataMap.get(workName);
    }
     ...
     * @param workName 工作名称
     */
    public static void removeEventMetadata(String workName) {
        eventMetadataMap.remove(workName);
    }
     ...
     */
    public MetadataEvent(String workName, int ddlWait, int maxQueueSizeOfOplog, int bucketNum, int bucketSize) {
        ...
        eventMetadataMap.put(workName, this);
    }
     ...
     */
    private BlockingQueue<ChangeStreamDocument<Document>> queueOfEvent;
    /**
     * 保存每个表的document的阻塞队列的映射
     * 键为表名，值为ns解析后的Document
     */
    private final Map<String, BlockingQueue<ChangeStreamDocument<Document>>> queueOfNsMap = new ConcurrentHashMap<>();
    /**
     * 保存每个表的状态的映射
     ...
     */
    private final Map<String, ChangeStreamDocument<Document>> currentNsDealOplogInfo = new ConcurrentHashMap<>();
    /**
     * 保存每个ns含有唯一索引个数的映射
     * 键为ns名称，值为唯一索引的个数
     */
    ...
    public int cacheQueueOfNsDataNum() {
        int sum = 0;
        for (Map.Entry<String, BlockingQueue<ChangeStreamDocument<Document>>> queueEntry : queueOfNsMap.entrySet()) {
    ...
    private void waitOplogNsBucketTask() {
        ...
        while (true) {
            for (Map.Entry<String, BlockingQueue<ChangeStreamDocument<Document>>> next : queueOfNsMap.entrySet()) {
                // 表名
                String dbTableName = next.getKey();
                BlockingQueue<ChangeStreamDocument<Document>> queue = next.getValue();
    ...
    public long printCacheInfo(long workStartTime, long executeCountOld) {
        try {
            ...
            log.info("{} the current number of real-time synchronization caches:{}", workName, eventMetadataMap.size());
            ...
            for (Map.Entry<String, ChangeStreamDocument<Document>> documentEntry : currentNsDealOplogInfo.entrySet()) {
            ...
            if (Calendar.getInstance().get(Calendar.SECOND) <= 10) {
                // 输出ns的缓存数量
                for (Map.Entry<String, BlockingQueue<ChangeStreamDocument<Document>>> entry : queueOfNsMap.entrySet()) {
